...
namespace ts.codefix {
    ...
    function tryDeleteFullVariableStatement(changes: textChanges.ChangeTracker, sourceFile: SourceFile, startToken: Node, deletedAncestors: NodeSet | undefined) {
        const varStatement = tryCast(startToken.parent.parent, isVariableStatement);
        if (varStatement && varStatement.declarationList.getChildren(sourceFile)[0] === startToken) {
            if (deletedAncestors) deletedAncestors.add(varStatement);
            changes.deleteNode(sourceFile, varStatement);
            return true;
        }
        return false;
    }
